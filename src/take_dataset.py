...
from ballistic_calc import calc_horizontal_travel_dist # type: ignore
...
# ================= CAMERA THREAD =================
def camera_thread_function(stop_event):
    global is_capturing
    save_path = "/home/pi/drone_dataset"
    ...
    # Fokus Infinity untuk Fixed-wing
    ...
    try:
        while not stop_event.is_set():
            if is_capturing:
                ...
                filename = os.path.join(save_path, f"WP_img_{ts}.jpg")
                
                picam2.capture_file(filename)
                
                # Interval 0.5 detik (2 fps)
                elapsed = time.time() - start_time
                time.sleep(max(0, 0.5 - elapsed))
            else:
                # Idle jika belum masuk area WP yang ditentukan
                time.sleep(0.1)
    finally:
        picam2.stop()
...
def main():
    ...
    cam_thread = threading.Thread(target=camera_thread_function, args=(stop_event,))
    cam_thread.start()
        ...
        while True:
            ...
            if state.connection_status:
                ...
                if (wp_passed + 1) in APPROACH:
                    ...
                    # Wahana sudah melewati WP sebelum Approach, mulai mengambil gambar
                    if not is_capturing:
                        is_capturing = True
                ...
                if wp_passed in APPROACH and mode == 'AUTO':
                    ...
                    # Memanggil fungsi dari ballistic_calc.py
                ...
                # Jika kita melewati WP Drop Zone tapi payload belum jatuh
                elif wp_passed in DROPPING:
                    # Pastikan WP ini adalah target dari approach terakhir
                    if wp_passed == current_drop_target and not dropped:
                    ...
                    # Wahana sudah melewati WP Drop Zone, stop mengambil gambar
                    if is_capturing:
                        is_capturing = False
